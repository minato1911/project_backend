...
function assumeTicket(idx){
  ...
  actionIdx=idx;
  const d=TR[currentLang]||TR['pt-BR'];
  const ci=document.getElementById('confirm-ico'); if(ci) ci.className='confirm-ico assume';
  const cii=document.getElementById('confirm-icon-i'); if(cii) cii.className='fa-solid fa-hand-pointer';
  const ct=document.getElementById('confirm-title'); if(ct) ct.textContent=d.confirm_assume_title;
  const cm=document.getElementById('confirm-msg'); if(cm) cm.textContent=d.confirm_assume_msg;
  const btn=document.getElementById('btn-confirm-action'); if(btn){ btn.className='btn-cd gld'; btn.textContent=d.btn_confirm; }
  btn.onclick=async ()=>{
    try {
      const t=tickets[actionIdx];
      ...
      timeline.unshift({type:'assume',text:`<strong>${currentUser?currentUser.nome:'Admin'}</strong> assumiu o chamado <strong>#${chamadoId}</strong>`,time:'Agora'});
      closeModal('modal-confirm'); renderAll(); showToast(d.toast_assumed,'info');
    } catch(e) {
      showToast('Erro ao assumir: ' + e.message, 'err');
    }
  };
  const mc=document.getElementById('modal-confirm'); if(mc) mc.classList.add('open');
}
...
function finishTicket(idx){
  ...
  actionIdx=idx;
  const d=TR[currentLang]||TR['pt-BR'];
  const ci=document.getElementById('confirm-ico'); if(ci) ci.className='confirm-ico finish';
  const cii=document.getElementById('confirm-icon-i'); if(cii) cii.className='fa-solid fa-check-circle';
  const ct=document.getElementById('confirm-title'); if(ct) ct.textContent=d.confirm_finish_title;
  const cm=document.getElementById('confirm-msg'); if(cm) cm.textContent=d.confirm_finish_msg;
  const btn=document.getElementById('btn-confirm-action'); if(btn){ btn.className='btn-cd ok'; btn.textContent=d.btn_confirm; }
  btn.onclick=async ()=>{
    try {
      const t=tickets[actionIdx];
      const chamadoId = typeof t.id === 'string' ? parseInt(t.id.replace('#','')) : t.id;
      ...
      timeline.unshift({type:'finish',text:`Chamado <strong>#${chamadoId}</strong> finalizado por <strong>${currentUser?currentUser.nome:'Admin'}</strong>`,time:'Agora'});
      closeModal('modal-confirm'); renderAll(); showToast(d.toast_finished,'suc');
    } catch(e) {
...
async function initAll(){
  initTheme(); initLang(); startLiveClock();
  await loadSession();
  await loadData();
}
initAll();